...
const dyn = @import("modules/dynamics.zig");
...
export fn process_limiter(ptr: [*]f32, len: usize, sample_rate: f32, threshold: f32, release: f32) void {
    dyn.processLimiter(ptr[0..len], sample_rate, threshold, release);
}

export fn process_de_esser(
    ptr: [*]f32, len: usize, sample_rate: f32,
    freq: f32, threshold: f32, ratio: f32, attack: f32, release: f32
) void {
    dyn.processDeesser(ptr[0..len], sample_rate, freq, threshold, ratio, attack, release);
}

export fn process_transient_shaper(ptr: [*]f32, len: usize, sample_rate: f32, att_db: f32, sus_db: f32, mix: f32) void {
    dyn.processTransientShaper(ptr[0..len], sample_rate, att_db, sus_db, mix);
}

// --- Time Modules ---

export fn process_chorus(
    ptr: [*]f32, len: usize, sample_rate: f32,
    rate: f32, base_time: f32, depth: f32, feedback: f32, wet: f32
) void {
    time.processChorus(ptr[0..len], sample_rate, rate, base_time, depth, feedback, wet);
}

export fn process_feedback_delay(
    ptr: [*]f32, len: usize, sample_rate: f32,
    t: f32, feedback: f32, wet: f32
) void {
    time.processFeedbackDelay(ptr[0..len], sample_rate, t, feedback, wet);
}

// --- EQ Modules ---

export fn process_parametric_eq(
    ptr: [*]f32, len: usize, sample_rate: f32,
    l_f: f32, l_g: f32, m_f: f32, m_g: f32, m_q: f32, h_f: f32, h_g: f32
) void {
    eq.processParametricEQ(ptr[0..len], sample_rate, l_f, l_g, m_f, m_g, m_q, h_f, h_g);
}

export fn process_midside_eq(
    ptr: [*]f32, len: usize, sample_rate: f32,
    m_g: f32, m_f: f32, s_g: f32, s_f: f32
) void {
    eq.processMidSideEQ(ptr[0..len], sample_rate, m_g, m_f, s_g, s_f);
}

export fn process_stereo_imager(
    ptr: [*]f32, len: usize, sample_rate: f32,
    l_f: f32, h_f: f32, w_l: f32, w_m: f32, w_h: f32
) void {
    eq.processStereoImager(ptr[0..len], sample_rate, l_f, h_f, w_l, w_m, w_h);
}

// --- Modulation Modules ---

export fn process_tremolo(ptr: [*]f32, len: usize, sample_rate: f32, freq: f32, depth: f32, wave: i32, mix: f32) void {
    modulation.processTremolo(ptr[0..len], sample_rate, freq, depth, wave, mix);
}

export fn process_phaser(
    ptr: [*]f32, len: usize, sample_rate: f32, 
    stages: i32, freq: f32, base: f32, oct: f32, wet: f32
) void {
    modulation.processPhaser(ptr[0..len], sample_rate, stages, freq, base, oct, wet);
}

// --- Creative Modules ---

export fn process_saturation(ptr: [*]f32, len: usize, drive: f32, sat_type: i32, out_gain_db: f32, mix: f32) void {
    creative.processSaturation(ptr[0..len], drive, sat_type, out_gain_db, mix);
}

export fn process_distortion(ptr: [*]f32, len: usize, drive: f32, dist_type: i32, out_gain_db: f32, mix: f32) void {
    creative.processDistortion(ptr[0..len], drive, dist_type, out_gain_db, mix);
}

export fn process_bitcrusher(ptr: [*]f32, len: usize, bits: f32, norm_freq: f32, mix: f32) void {
    creative.processBitcrusher(ptr[0..len], bits, norm_freq, mix);
}

export fn process_dithering(ptr: [*]f32, len: usize, bits: f32) void {
...
const Weighting = struct {
    // Advanced Psycho-acoustic weighting (Inverse Fletcher-Munson approximation)
    // Combines A-weighting with ear canal resonance correction (~3.5kHz)
    
    fn get_factor(freq: f32) f32 {
        if (freq < 20.0) return 0.0001;
        
        // Base A-Weighting
        const f2 = freq * freq;
        const f4 = f2 * f2;
        const num = 12194.0 * 12194.0 * f4;
        const den = (f2 + 20.6 * 20.6) * 
                    std.math.sqrt((f2 + 107.7 * 107.7) * (f2 + 737.9 * 737.9)) * 
                    (f2 + 12194.0 * 12194.0);
        var ra = num / den;
        
        // Normalize ra to 1kHz (approx 0.7943)
        ra /= 0.7943;

        // Ear Canal Resonance Peak (approx +3-4dB @ 3.5kHz)
        // We model this as a simple bell-like boost
        const resonance_f = 3500.0;
        const resonance_q = 1.0;
        const dist = std.math.log10(freq / resonance_f);
        const resonance_boost = 1.5 * std.math.exp(- (dist * dist) / (resonance_q * resonance_q));
        
        return ra * (1.0 + resonance_boost);
    }
};
...
// --- Tonal Health Intelligence ---

export fn analyze_tonal_health(ptr_mags: [*]f32, num_bins: usize, sample_rate: f32, out_ptr: [*]f32) void {
    const mags = ptr_mags[0..num_bins];
    const bin_size = (sample_rate / 2.0) / @as(f32, @floatFromInt(num_bins));

    var total_energy: f32 = 0;
    for (mags) |m| total_energy += m * m;
    total_energy += 1e-10;

    // 1. Muddiness Detector (200-500Hz)
    var mud_energy: f32 = 0;
    // 2. Sibilance Detector (5kHz-10kHz)
    var sib_energy: f32 = 0;
    // 3. Harshness Detector (2kHz-4kHz)
    var harsh_energy: f32 = 0;

    for (mags, 0..) |m, i| {
        const f = @as(f32, @floatFromInt(i)) * bin_size;
        const e = m * m;
        if (f >= 200 and f <= 500) mud_energy += e;
        if (f >= 2000 and f <= 4000) harsh_energy += e;
        if (f >= 5000 and f <= 10000) sib_energy += e;
    }

    // 4. Resonance Detection (finding sharp peaks)
    var peak_count: f32 = 0;
    var max_resonance: f32 = 0;
    for (1..num_bins-1) |i| {
        if (mags[i] > mags[i-1] * 2.0 and mags[i] > mags[i+1] * 2.0) {
            peak_count += 1.0;
            const res = mags[i] / ((mags[i-1] + mags[i+1]) * 0.5);
            if (res > max_resonance) max_resonance = res;
        }
    }

    out_ptr[0] = mud_energy / total_energy;    // Mud Ratio
    out_ptr[1] = harsh_energy / total_energy;  // Harshness Ratio
    out_ptr[2] = sib_energy / total_energy;    // Sibilance Ratio
    out_ptr[3] = peak_count;                  // Number of sharp peaks
    out_ptr[4] = max_resonance;               // Highest resonance factor
}

// --- 7. Real-time Visualization Helpers ---

export fn process_fft_magnitudes(ptr: [*]f32, len: usize, out_ptr: [*]f32) void {
    const data = ptr[0..len];
    
    // Allocate complex buffer for FFT
    const fft_buf = allocator.alloc(math.Complex, len) catch return;
    defer allocator.free(fft_buf);
    
    // 1. Apply Hanning window and prepare complex buffer
    // Hanning window is best for general purpose spectral analysis
    const f_len = @as(f32, @floatFromInt(len));
    for (data, 0..) |sample, idx| {
        const w = 0.5 * (1.0 - std.math.cos(math.TWO_PI * @as(f32, @floatFromInt(idx)) / (f_len - 1.0)));
        fft_buf[idx] = .{ .re = sample * w, .im = 0 };
    }
    
    // 2. FFT
    math.fft_iterative(fft_buf, false);
    
    // 3. Magnitudes (first half / Nyquist)
    // Scale by 2/N to represent amplitude correctly.
    // Also apply a psycho-acoustic weighting (Inverse Fletcher-Munson / A-Weighting)
    const num_bins = len / 2;
    const scale = (2.0 / f_len) * 2.0; 
    const bin_size = 48000.0 / f_len; // Assuming 48k for mapping if sample rate not passed
    
    for (0..num_bins) |k| {
        const freq = @as(f32, @floatFromInt(k)) * bin_size;
        const w = Weighting.get_factor(freq);
        out_ptr[k] = fft_buf[k].magnitude() * scale * w;
    }
}

export fn process_metering(ptr: [*]f32, len: usize, channels: i32, out_ptr: [*]f32) void {
    const data = ptr[0..len];
    const chan_count: usize = if (channels >= 2) @as(usize, 2) else @as(usize, 1);
    ...
    // Simple 1-pole Lowpass at 100Hz to slightly dampen the meter ballistics?
    // Actually, for "Fletcher-Munson" weighting on meters, we should use a filter.
    // But since we are doing block-based calculation, we can apply a simple RLB-style filter
    // if we want to be very accurate. For now, let's at least do weighted RMS.
    
    var i: usize = 0;
    while (i < len) : (i += chan_count) {
        const l = data[i];
        const r = if (chan_count == 2) data[i+1] else l;
        
        // Use peak of raw signal
        if (@abs(l) > peak_l) peak_l = @abs(l);
        if (@abs(r) > peak_r) peak_r = @abs(r);
        
        // For RMS, we could apply a filter, but block-average is standard.
        // We'll return raw RMS and the UI can handle weighting or we can use LUFS.
        sum_l += l * l;
        sum_r += r * r;
    }
    
    const count = @as(f64, @floatFromInt(len / chan_count));
    out_ptr[0] = @floatCast(std.math.sqrt(sum_l / count));
    out_ptr[1] = @floatCast(std.math.sqrt(sum_r / count));
    out_ptr[2] = peak_l;
    out_ptr[3] = peak_r;
}
