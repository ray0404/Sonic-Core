export type RackModuleType = 'DYNAMIC_EQ' | 'TRANSIENT_SHAPER' | 'LIMITER' | 'MIDSIDE_EQ' | 'CAB_SIM' | 'LOUDNESS_METER' | 'SATURATION' | 'DITHERING' | 'PARAMETRIC_EQ' | 'DISTORTION' | 'BITCRUSHER' | 'CHORUS' | 'PHASER' | 'TREMOLO' | 'AUTOWAH' | 'FEEDBACK_DELAY' | 'COMPRESSOR' | 'DE_ESSER' | 'STEREO_IMAGER' | 'MULTIBAND_COMPRESSOR' |
  'DE_CLIP' | 'PHASE_ROTATION' | 'SPECTRAL_DENOISE' | 'MONO_BASS' | 'PLOSIVE_GUARD' | 'VOICE_ISOLATE' | 'SMART_LEVEL' | 'DE_BLEED' | 'TAPE_STABILIZER' | 'ECHO_VANISH' | 'PSYCHO_DYNAMIC_EQ' |
  'ZIG_COMPRESSOR' | 'ZIG_SATURATION' | 'ZIG_LIMITER' | 'ZIG_DE_ESSER' | 'ZIG_TRANSIENT_SHAPER' | 'ZIG_BITCRUSHER' | 'SPECTRAL_MATCH' | 'LUFS_NORMALIZER' |
  'GUITAR_RIG' | 'DRUM_MACHINE' | 'TAB_PLAYER' | 'METRONOME' | 'TUNER' |
  'PEDAL_CHORUS' | 'PEDAL_COMPRESSOR' | 'PEDAL_DELAY' | 'PEDAL_OVERDRIVE' | 'PEDAL_REVERB' | 'PEDAL_SUPERNOVA' | 'PEDAL_TREMOLO';
